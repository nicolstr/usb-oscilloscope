...
{
	TIM4->SR &= ~(1 << 0);  	// clear TIM4 pending flag (UIF)
...
void EXTI15_10_IRQHandler()
{
	...
	TIM4->CR1 |= TIM_CR1_CEN;       // enable Counter TIM4
	TIM1->CR1 |= TIM_CR1_CEN;       // enable Counter TIM1
}

/* exception handler for DMA2 Stream5 */
void DMA2_Stream5_IRQHandler()
{
	DMA2->HISR &= ~(DMA_HISR_TEIF5);	// clear transfer error Interrupt Flag
	int test1 = 1;
}
